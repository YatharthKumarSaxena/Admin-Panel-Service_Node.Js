...
/**
 * Service: Reject Activation Request
 * Rejects activation request atomically
 * 
 * @param {Object} request - The activation request document
 ...
 */
const rejectActivationRequestService = async (
    request,
    actor,
    reviewNotes,
    device,
    ...
    try {
        ...
        if (!updatedRequest) {
            return {
                success: false,
                type: 'UPDATE_FAILED',
                message: 'Failed to reject activation request'
            };
        }

        logWithTime(`✅ Activation request ${request.requestId} rejected by ${actor.adminId}`);
        ...
        await logActivityTrackerEvent(
            ...
            ACTIVITY_TRACKER_EVENTS.REJECT_ACTIVATION_REQUEST,
            {
                description: `Activation request ${request.requestId} rejected by ${actor.adminId}`,
                adminActions: {
        ...
        logWithTime(`❌ Error in rejectActivationRequestService: ${error.message}`);
        return {
            success: false,
            message: error.message || 'Failed to reject activation request'
        };
    }
};
